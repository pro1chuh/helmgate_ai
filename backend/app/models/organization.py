...
from sqlalchemy import String, Text, ForeignKey, Integer, Numeric, Enum as SAEnum, Boolean, JSON
...
class Webhook(Base):
    """
    Webhook-подписка организации.
    При наступлении события Helm делает POST на указанный URL с JSON-payload.

    Поддерживаемые события (events):
      - message.created  — каждое сообщение ассистента
      - balance.low      — баланс упал ниже порога
      - balance.exhausted — баланс обнулён, аккаунт приостановлен
    ...
    # Список событий в JSON-массиве: ["message.created", "balance.low"]
    events: Mapped[list] = mapped_column(JSON, default=list)
    # HMAC-секрет для верификации: X-Helm-Signature = HMAC-SHA256(body, secret)
