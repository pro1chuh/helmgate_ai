function md(text) {
  if (!text) return "";
  const normalized = normalizeImageContent(text);
  if (window.marked) {
    try {
...
function normalizeImageContent(text) {
  const content = String(text || "").trim();
  if (/^!\[[^\]]*\]\((data:image\/|https?:\/\/)/i.test(content)) {
    return content;
  }
  if (/^data:image\/[^;\s]+;base64,/i.test(content)) {
    return `![generated image](${content})`;
  }
  if (/^https?:\/\/\S+\.(png|jpe?g|gif|webp|bmp|svg)(\?\S*)?$/i.test(content)) {
    return `![generated image](${content})`;
  }
  return text;
}
